 ...
 *      Initialization, mode setting, CAN communication bit timing setting, message transmission,
 *      message reception, error check, ID conversion function for CAN communication initialization.
 *
 */
...
uint8_t CANSPI_Transmit(uCAN_MSG *tempCanMsg)
{
  ...
  if (ctrlStatus.TXB0REQ != 1)
  {
    ...
    /* Loading data to be sent to Tx Buffer */
    MCP2515_LoadTxSequence(MCP2515_LOAD_TXB0SIDH, &(idReg.tempSIDH), tempCanMsg->frame.dlc, &(tempCanMsg->frame.data0));

    /* Tx Buffer transmission request */
...
uint8_t CANSPI_Receive(uCAN_MSG *tempCanMsg)
{
  ...
  {
    /* Check which buffer contains the message and process it */
...
/* Check if Rx Passive Error status is in */
